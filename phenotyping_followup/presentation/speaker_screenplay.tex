...
\title{\sffamily\bfseries SENSORIMOTOR HABITUATION\\IN \textit{DROSOPHILA} LARVAE\\[1em]\large A Speaker's Screenplay}
\author{Gil Raitses\\Syracuse University}
\date{Bhattacharya Lab General Meeting\\December 2025}
...
\dialogue{What I want to do today is walk through how I've been thinking about this data, rather than just show results. We collect complex behavioral data in the lab, and the question I've been asking is how far we can push interpretability before we start overreading structure. I'm using modeling, simulation, and validation the same way we use controls in experiments, to figure out what kinds of explanations are justified at different scales.}

\direction{Small pause. Let the frame settle.}

\dialogue{What you'll see is that this works really well at the population level. We get clean, interpretable structure that lines up nicely with how we already think about sensory response and adaptation. But what's more interesting to me is what happens when we try to push the same ideas down to individuals. That's where things stop working in a useful way, and that failure turns out to be very informative.}

\cue{Advance to Slide 1}

\dialogue{So I'm going to start at the population level, where everything behaves nicely, and then I'll move to the individual level to show where the boundaries are and how we might push them experimentally.}
...
\dialogue{At this point, everything is still very comfortable. The population data are dense, the structure is clear, and the model behaves the way we'd hope.}

\cue{Advance to Slide 8}

\dialogue{Here's one example of that. Across repeated light pulses, turning behavior gradually increases. That slow change is exactly what the later part of the kernel is capturing. So the model isn't just fitting a curve, it's tracking something we already recognize as habituation.}
...
\dialogue{This is not because the clustering algorithm is bad. It's because we're asking more of the data than they can support.}
...
\dialogue{Once the individual level fails, the question stops being how do we fit this model, and becomes what would we need to change experimentally for this to work at all.}
...
\dialogue{So at this point, the model is telling us something important. When we try to estimate individual parameters, almost everything collapses back to the population mean. That's not a failure of the statistics. It's a sign that the individual data just don't contain enough information.}

\cue{Advance to Slide 15}

\dialogue{This slide is about why that happens.}

\dialogue{What matters here isn't just how many events we collect, but when those events occur relative to the stimulus. Some events are informative about the fast response, and others really aren't.}
...
\dialogue{This is the key idea. It's not that burst stimulation magically creates data. It just makes each event more useful for the question we're asking.}
...
\dialogue{Once you frame it this way, you can actually ask practical questions. How many events would we need. How long would we need to record. What kind of protocol would make individual differences detectable at all.}
...
\dialogue{What I find useful about this is that it tells us where effort is best spent. Instead of forcing increasingly complex models onto sparse data, we can either simplify what we try to estimate or redesign the experiment to extract information more efficiently.}
...
\dialogue{I don't want to give a sloppy answer to that, but I can tell you what the analysis says at a high level and we can follow up on details afterward.}
...
\dialogue{The main point I wanted to make today is that the limits we're seeing are not failures of modeling, they're measurements of where our experiments stop being informative.}
