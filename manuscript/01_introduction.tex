...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{data/figures/figure1_kernel.png}
\caption{\textbf{Analytic hazard kernel decomposition.} The stimulus-onset response is modeled as a difference of two gamma probability density functions, enabling direct interpretation of temporal dynamics. \textbf{(A)} Full kernel reconstruction: the analytic gamma-difference form K$_{\text{on}}$(t) shown as a blue solid line. The red shaded region indicates the suppression phase where K$_{\text{on}}$(t) $< 0$. Peak suppression of $-3.06$ occurs at $t^* = 2.9$ s. The kernel shows characteristic biphasic dynamics with an initial transient followed by sustained suppression. \textbf{(B)} Component decomposition: the fast excitatory component shown in green with $\alpha_1 = 2.22$, $\beta_1 = 0.132$ s, and mean $\tau_1 = 0.29$ s peaks early and captures rapid sensory transduction. The slow suppressive component shown in red with $\alpha_2 = 4.38$, $\beta_2 = 0.869$ s, and mean $\tau_2 = 3.81$ s peaks at $\sim$2.9 s and represents synaptic adaptation. The combined kernel shown as a blue dashed line matches Panel A. Vertical dashed lines indicate peak times for each component.}
\label{fig:kernel}
\end{figure}
