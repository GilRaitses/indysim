#!/usr/bin/env python3
"""
INDYsim Demo Pipeline - Full Analysis on Single H5 File
...
Usage:
    python scripts/indysim_demo.py <h5_file> [--output-dir <dir>]
    
Example:
    python scripts/indysim_demo.py \
...
def step6_generate_figures(output_dir: Path, traj_file: Path, model_results: dict, rc_results: dict):
    ...
    if 'x' in df.columns and 'y' in df.columns:
        ...
        ax.set_title('Trajectory with Reversals (purple)', fontsize=12, fontweight='bold', color='white')
    ...
    if 'is_reorientation' in df.columns and 'time_since_stimulus' in df.columns:
        ...
        ax.set_title('Peristimulus Turn Rate', fontsize=14, fontweight='bold')
        ax.legend()
    ...
    if 'speed_run_vel' in df.columns and 'time' in df.columns:
        ...
        ax.set_title('Dot Product of Heading × Velocity', fontsize=12, fontweight='bold')
...
This report presents the results of running the INDYsim hazard model pipeline
...
*Report generated by INDYsim Demo Pipeline*
'''
...
def main():
    parser = argparse.ArgumentParser(
        description="INDYsim Demo Pipeline - Full analysis on single H5 file",
